        ...
        <div style={{ padding: 12 }}>
            ...
            <div style={{ marginTop: 12 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
                <div style={{ padding: 4, background: "#f3f4f6", borderRadius: 4, textAlign: "center", fontSize: 11 }}>
                  <div style={{ fontWeight: 700 }}>{filesCount}</div><div>{t("Files")}</div></div>
                <div style={{ padding: 4, background: "#f3f4f6", borderRadius: 4, textAlign: "center", fontSize: 11 }}>
                  <div style={{ fontWeight: 700 }}>{findings.length}</div><div>{t("Findings")}</div></div>
                <div style={{ padding: 4, background: sevCounts.critical > 0 ? "#fee2e2" : "#f3f4f6", borderRadius: 4, textAlign: "center", fontSize: 11 }}>
                  <div style={{ fontWeight: 700, color: sevCounts.critical > 0 ? "#dc2626" : "#374151" }}>{sevCounts.critical}</div><div>{t("Critical")}</div></div>
                <div style={{ padding: 4, background: sevCounts.high > 0 ? "#ffedd5" : "#f3f4f6", borderRadius: 4, textAlign: "center", fontSize: 11 }}>
                  <div style={{ fontWeight: 700, color: sevCounts.high > 0 ? "#ea580c" : "#374151" }}>{sevCounts.high}</div><div>{t("High")}</div></div>
              </div>
              {riskLevel && (
                <div style={{ marginTop: 8, padding: 6, borderRadius: 6, fontSize: 12, textAlign: "center",
                  ...
                  {t("Risk")}: {riskScore}/100 ({riskLevel})
                </div>
              )}
          ...
          {Object.keys(timing).length > 0 && (
            <div style={{ marginTop: 8, fontSize: 10, color: "#9ca3af" }}>
              {t("Fetch")}: {timing.fetch}s | {t("Analyze")}: {timing.analyze}s | {t("Total")}: {timing.total}s
            </div>
          )}
        </div>
      ...
      <main style={{ flex: 1, overflow: "auto", padding: 16 }}>
        ...
        {loading && (
          <div style={{ textAlign: "center", padding: 80 }}>
            ...
            <h3 style={{ color: "#374151", marginBottom: 8 }}>{t("Analyzing Pull Request")}</h3>
            <p style={{ color: "#6b7280", fontSize: 14 }}>{t("Running SAST + LLM analysis with 6 analyzers…")}</p>
            <p style={{ color: "#9ca3af", fontSize: 12, marginTop: 16 }}>{t("This may take 20-60 seconds depending on PR size.")}</p>
          </div>
        )}
        {!loading && findings.length === 0 && !error && (
          <div style={{ textAlign: "center", padding: 80, color: "#9ca3af" }}>
            <h2>AI PR Reviewer</h2>
            <p>{t("Select categories and mode in the sidebar, then click \"Run Analysis\".")}</p>
            <p style={{ fontSize: 13 }}>{t("SAST (Bandit/ESLint/staticcheck/Semgrep) + LLM analysis with 6 independent analyzers.")}</p>
          </div>
        )}
        {summary && (
