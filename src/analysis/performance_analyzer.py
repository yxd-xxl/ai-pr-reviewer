"""Performance analyzer — complexity, I/O, memory, concurrency."""
...
class PerformanceAnalyzer(Analyzer):
    """Independent analyzer for performance issues."""
            ...
            with ThreadPoolExecutor(max_workers=min(self._parallel, len(analyzable))) as pool:
                ...
                for f in as_completed(futures):
                    try:
                        ...
                        errors.append(f"Perf analysis failed: {e}")

        # Verify critical/high
        verify_sevs = ("critical", "high") if not self._verify_all else ("critical", "high", "medium")
        to_verify = [f for f in findings if f.severity in verify_sevs and f.confidence >= 0.5]
        verified, rejected = [], 0
        if to_verify:
            with ThreadPoolExecutor(max_workers=min(4, len(to_verify))) as pool:
                ...
                for fut in as_completed(futures):
                    ...
                    if ok and conf >= 0.5:
                        ...
                        warnings.append(f"Perf verify rejected: {f.title}"); rejected += 1
            if rejected:
                warnings.append(f"Perf verification filtered {rejected} finding(s)")
        for f in findings:
            if f not in to_verify:
                verified.append(f)

        return ReviewResult(summary=summary, findings=verified,
                           metadata={"analyzer": "performance", "model": "llm"},
    ...
    def _summary(self, ctx, warnings, errors):
        try:
            ...
            errors.append(f"Perf summary failed: {e}")
            return "## Performance Analysis\n\nFailed."

    def _file(self, fc, ctx, sast):
        s, u = build_performance_prompt(fc, ctx, sast)
        data = self._adapter.complete_json(system=s, user=u)
        return [Finding(
            severity=f.get("severity", "medium"), category="performance",
            ...
            evidence=f.get("evidence"), analyzer="performance",
