...
> **B 站链接**: [待录制上传]
> 
> 视频内容：完整演示从连接 GitHub → 选择仓库 → Review PR → 查看 Finding → 批量 Review → Fix PR 的全流程。
> 请确保视频可公开访问，评委可直接在浏览器中播放。
...
| **多语言 SAST** | Python(Bandit) + JS/TS(ESLint) + Go(staticcheck) + Semgrep 跨语言 |
| **6 维独立分析器** | security/bug/performance/style/architecture/failure 各独立 prompt |
| **独立验证** | critical/high finding 经第二 LLM 独立验证，过滤误报 |
| **自动 Fix PR** | 生成 fix patch → 安全检查(apply/syntax/destructive) → Fix PR |
| **批量 Review** | `batch review` CLI + Bulk Review Center UI，并行评审 |
| **Feedback 系统** | 10 种状态标记(tp/fp/wont_fix/duplicate/fixed...)，反馈学习 |
| **多维 Risk Score** | 6 维分解：基础/安全/变更/测试缺口/上下文/证据质量 |
...
| **GitHub Action** | PR opened/synchronize 自动触发 + risk label |
| **Streamlit Dashboard** | 5 Stage 浏览器 UI（含 Bulk Review Center）|
...
```bash
python -m pytest tests/ -v    # 136 tests (mock mode), 11 eval cases (DeepSeek)
```
...
| eslint | JS/TS SAST | 可选 |
| staticcheck | Go SAST | 可选 |
| PyJWT | GitHub App 认证 | 可选 |
| sentence-transformers | RAG 向量化 | 可选 |
